...
import { ActivitySquare, Search, Filter, Package, Divide, ListPlus, Calendar } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-card/50 backdrop-blur-sm border-ana-purple/30">
        <CardHeader>
          ...
          <CardDescription>
            Track all AirNode NFT minting, fractionalization, and marketplace listing activities
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {filteredActivity.length === 0 ? (
              ...
              filteredActivity.map(activity => (
                ...
                >
                  <div className={`h-10 w-10 rounded-md flex items-center justify-center ${activityColors[activity.type]}`}>
          ...
          {filteredActivity.length > 0 && (
            <div className="mt-6 flex justify-between items-center text-sm text-muted-foreground">
              <span>Showing {filteredActivity.length} of {mockActivity.length} activities</span>
